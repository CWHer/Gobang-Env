...
private:
    ...
    std::shared_ptr<Gobang> gobang_env;
    int winner;

public:
    MCTS(float c_puct, int num_search, std::shared_ptr<Gobang> gobang_env)
        : c_puct(c_puct), num_search(num_search), current_search(0),
          root(std::make_shared<TreeNode>(std::weak_ptr<TreeNode>(), -1, 0, c_puct)),
          stat(gobang_env->getStat()), selected_node(nullptr), gobang_env(gobang_env)
    {
    }
    ...
    bool selectNode()
    {
        // MCTS: select
        selected_node = root;
        gobang_env->setStat(stat);
        while (!selected_node->isLeaf())
        {
            selected_node = selected_node->select();
            gobang_env->step(selected_node->action);
        }

        auto result = gobang_env->checkFinished();
    ...
    {
        // MCTS: expand
        auto valid_actions = gobang_env->getActions();
